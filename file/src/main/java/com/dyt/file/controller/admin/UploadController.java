...
import com.dyt.server.enums.FileUseEnum;
import com.dyt.server.service.FileService;
import com.dyt.server.util.Base64ToMultipartFile;
...
public class UploadController {
    ...
    public ResponseDto merge() throws Exception {
        ...
        try {
            // 读取第一个分片
            fileInputStream = new FileInputStream(new File(FILE_PATH + "/course/bVqILzSt.blob"));
            while ((len = fileInputStream.read(byt)) != -1) {
                outputStream.write(byt, 0, len);
            }

            // 读取第二个分片
            fileInputStream = new FileInputStream(new File(FILE_PATH + "/course/IxLmlXuk.blob"));
            while ((len = fileInputStream.read(byt)) != -1) {
                outputStream.write(byt, 0, len);
            }
        } catch (IOException e) {
